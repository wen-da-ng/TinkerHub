# C:\Users\L\Desktop\Dev\MCP_learnings\mySecondAdvancedLLM\api_server.py
...
async def analyze_multiple_documents_with_code(provider, filenames: List[str], question: str) -> str:
    """Analyze multiple documents using code generation."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Copy each document to the temp directory with a clear naming scheme
        file_paths = []
        for i, doc_name in enumerate(filenames):
            complete_doc = retrieve_complete_document(doc_name)
            if complete_doc:
                # Create a valid filename that preserves the original name but is safe for the filesystem
                safe_name = re.sub(r'[^\w\-\.]', '_', doc_name)
                # Use a consistent extension (.txt) for all files
                if "." in safe_name:
                    base_name = safe_name.rsplit('.', 1)[0]
                    file_path = os.path.join(temp_dir, f"doc_{i+1}_{base_name}.txt")
                else:
                    file_path = os.path.join(temp_dir, f"doc_{i+1}_{safe_name}.txt")
                
                logger.info(f"Creating file for document '{doc_name}' at '{file_path}'")
                
                with open(file_path, 'w', encoding='utf-8') as f:
                    doc_content = "".join(complete_doc)
                    f.write(doc_content)
                    logger.info(f"Wrote {len(doc_content)} characters to {file_path}")
        ...
        # Create a manifest file that lists all documents
        manifest_path = os.path.join(temp_dir, "document_manifest.json")
        document_info = [
            {
                "id": i+1,
        ...
        # Sample the first few lines of each document
        samples = []
        for doc_info in document_info:
            path = doc_info["path"]
            with open(path, 'r', encoding='utf-8') as f:
                sample = f.read(300)  # First 300 chars
                samples.append(f"File: {doc_info['filename']} (original: {doc_info['original_name']})\nSample: {sample}...")
        
        # Create a detailed instruction for the code generation
        code_instructions = f"""
You need to analyze {len(filenames)} documents to answer this question: {question}

EXACT FILE PATHS - YOU MUST USE THESE EXACT PATHS:
- Manifest file: {manifest_path}
- Base directory: {temp_dir}

The manifest file contains information about all documents. Your code MUST:
1. HARDCODE the manifest path as: "{manifest_path}"
2. Load this manifest file using this exact path
3. Process each document listed in the manifest using the exact paths provided in the manifest
4. Do NOT ask for user input, do NOT use input() functions
5. Do NOT use relative paths or os.path.expanduser()
...
CRITICAL: Your code must start with loading the manifest from the exact path "{manifest_path}"
"""
        logger.info("Executing code-based analysis with detailed instructions")
        
        # Execute the analysis with code
        return await analyze_with_code(provider, manifest_path, code_instructions)
